...
pub struct BaseObjectContainer<T, InheritPtrs: Clone = ()>(
  pub(in super::super) Arc<BaseObjectWrapper<T, InheritPtrs>>,
);

impl<T, InheritPtrs: Clone> Deref for BaseObjectContainer<T, InheritPtrs> {
  type Target = Arc<BaseObjectWrapper<T, InheritPtrs>>;
...
impl<T, InheritPtrs: Clone> PartialEq for BaseObjectContainer<T, InheritPtrs> {
  fn eq(&self, other: &Self) -> bool {
    let Ok(borrow) = self.0.value.try_read() else {
      return false;
    };
    let Ok(other_borrow) = other.0.value.try_read() else {
      return false;
    };
