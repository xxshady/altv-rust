...
impl blip::Blip {
  ...
  pub fn new_global_entity_point(entity: impl Into<AnyEntity>) -> SomeResult<blip::BlipContainer> {
  ...
  pub fn targets(&self) -> SomeResult<Vec<player::PlayerContainer>> {
    ...
    Ok(Resource::with(|resource| {
      unsafe { sdk::IBlip::GetTargets(raw_ptr) }
        .into_iter()
        .map(|v| helpers::get_non_null_player(unsafe { sdk::read_player_ptr_wrapper(v) }, resource))
        .collect()
    }))
  }
  ...
  pub fn set_secondary_color(&self, color: impl Into<Rgba>) -> VoidResult {
    ...
    unsafe {
      sdk::IBlip::SetSecondaryColor(self.raw_ptr()?, color.r(), color.g(), color.b(), color.a())
    }
    Ok(())
  }
