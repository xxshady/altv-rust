  ...
  pub fn process_timers(&mut self, mut schedule: RwLockWriteGuard<ScheduleState>) {
    ...
    for (idx, timer) in self.timers.iter_mut().enumerate().rev() {
      if now >= timer.next_call_time {
        ...
        timer.next_call_time = time::SystemTime::now() + time::Duration::from_millis(timer.millis);
      }
    }
  ...
  fn get_to_be_destroyed(&self, schedule: &RwLockWriteGuard<ScheduleState>) -> Vec<usize> {
    let mut indexes: Vec<usize> = schedule
      ...
      .filter_map(|id| {
        ...
        if timer.is_none() {
          logger::warn!("Failed to destroy timer with id: {id} (it was probably already removed)");
        } else {
